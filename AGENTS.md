...
| `pnpm test` | Vitest `jsdom` `run` — **9 files / 53 tests** (`src/utils/cn` 5 + `data/nav` 7 + `data/content` 10 + `data/site` 6 + `ui/Button` 8 + `SkipLink` 3 + `ui/Accordion` 4 + `SafeImage` 6 + `Header` 4) |
| `pnpm test:watch` | Vitest watch mode |
| `pnpm test:coverage` | Vitest with coverage (`vitest run --coverage`) |
| `pnpm test:e2e` | Playwright `chromium` (4 specs / 22 tests) — `playwright.config.ts` + `e2e/` (smoke 8 + navigation 6 + ministries 4 + give-faq 4) |
...
src/ (42 files — 32 source + 9 tests + 1 setup)
  ...
  index.css            # @theme tokens (24 colors + 2 shadows) + @layer base/utilities (12 utilities: text-balance, bg-adobe-texture, bg-grain, divider-weave, divider-weave-thin, gold-rule, gold-rule-left, hero-ken-burns, mask-fade-b, reveal, reveal-visible, skip-link + 2 keyframes gold-rule-draw/hero-ken-burns)
  ...
  test/setup.ts        # vitest jsdom setup (jest-dom + IntersectionObserver mock + scrollTo/scrollIntoView stubs)
...
e2e/                   # 22 tests — smoke.spec.ts (8) + navigation.spec.ts (6) + ministries.spec.ts (4) + give-faq.spec.ts (4) + helpers.ts
...
src.orig/              # NOT present in this repository (never committed) — the Rother Shrine original is referenced by docs/history only; eslint + vite-watch ignore entries for it are kept as inert defensive guards
```
...
- **Test/lint harness** — `eslint 9.39.5` flat + `vitest 3.2.6` (jsdom) + `@testing-library/react 16.2.0` + `playwright 1.55.1` (chromium) — gate is `lint && typecheck && test && test:e2e && build` (`9 unit files / 53 tests` via `src/test/setup.ts` + `22 E2E` via `e2e/`). CI mirrors this in `.github/workflows/ci.yml` (Node 24, pnpm 11). `eslint` ignores `skills/` + `src.orig/` (inert for `src.orig/` — it is not in the repo); `vite.config.ts` `test` + `server.watch.ignored` + `tsconfig.json` `types [vitest/globals]` are required. Don't re-add `src.orig` to lint/tsc scope.
...
- **Header close-on-activation** — the mobile drawer closes on any in-drawer link activation (`onClickCapture` on the drawer `<nav>`), not only on pathname change: a tap on a link to the **current** route never changes `pathname`, so the `useEffect([pathname])` close cannot fire (H-1 in `docs/code-review-audit-2026-08-28.md` — fixed 2026-08-28; covered by `src/components/Header.test.tsx` + `e2e/smoke.spec.ts`). Desktop dropdown `<ul>` also closes on child-link click.
...
- `src.orig/` — **not part of this repository** (never committed; no snapshot exists here). The eslint/vite ignore entries are inert defensive guards; do not re-add to lint/tsc scope.
