...
pnpm test               # vitest jsdom — 16 files / 89 tests
...
- Tests: `*.test.{ts,tsx}` adjacent to source — **16 files / 89 tests**: `src/utils/cn.test.ts` (5), `src/data/nav.test.ts` (7), `src/data/content.test.ts` (10), `src/data/site.test.ts` (6), `src/components/ui/Button.test.tsx` (9), `src/components/SkipLink.test.tsx` (3), `src/components/ui/Accordion.test.tsx` (6), `src/components/SafeImage.test.tsx` (6), `src/components/Header.test.tsx` (11), `src/components/BackToTop.test.tsx` (6), `src/components/Layout.test.tsx` (2), `src/components/ScrollProgress.test.tsx` (2), `src/hooks/useScrollProgress.test.ts` (4), `src/pages/Ministries.test.tsx` (3), `src/pages/cta-bands.test.tsx` (3), `src/head.test.ts` (6) + `src/test/setup.ts` (jest-dom + IntersectionObserver mock + matchMedia stub). `vite.config.ts` `test.exclude` keeps `e2e/**` out of unit runs; `e2e/*.spec.ts` is Playwright only.
...
- `pnpm lint`, `pnpm typecheck`, `pnpm test` (16 files / 89 tests), `pnpm test:e2e` (35 tests, chromium), and `pnpm build` are all green (121 total — 16 unit files + 5 E2E specs via `lint && typecheck && test && test:e2e && build`).
...
| 5 | Testing Strategy (wired — 16 unit files / 89 tests + 35 E2E) | Yes | ✅ |
